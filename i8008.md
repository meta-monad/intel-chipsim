...
Instructions might take 1, 2, or 3 machine cycles to complete. First cycle is the *instruction fetch cycle* (PCI). The second and third are either for *data reading* (PCR), *data writing* (PCW), or *I/O operations* (PCC). The cycles are encoded in the D6 & D7 pins.
...
There is one pseudoregister, M, which most of the time can be used interchangeably with the scratchpad registers. M is, in effect, that particular byte in memory whose address is currently contained in H and L (L contains the eight lower bits of the address and H contains the six higher bits).
